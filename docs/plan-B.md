...
Audio VAD를 Track B의 기준 측정 방식으로 사용한다. STT word timestamp가 제공되더라도 필수 의존성으로 두지 않는다. 시간 필드는 내부 계산과 호환성 보존에만 사용하며, 외부 결과 화면에서는 질문별 오디오 활동 타임라인으로 시각화한다.
...
audio_timeline (energy / speech / long_pause, 최대 120개 구간)
~~~
...
- 사용자에게는 `Phase 1 Moving`(`+`·`X`) → `Phase 2 Static`(고정 3×3 9-point) 순서로 표시한다. 내부의 `settle`·`moving`·`preview`·`collecting` 상태와 사용자용 단계를 분리한다.
- 시작 전 준비 시간은 3초로 두고, Phase 1 진입 전에 중앙 점을 1500ms 동안 흐릿하게 깜빡이며 “끝까지 따라갔다가 중앙으로 되돌아오는” 동작을 안내한다. 이 안내 구간에는 샘플을 수집하지 않는다.
- Phase 1의 `+`와 `X`는 중앙에서 바깥 방향으로 출발할 때만 현재 점 주변에 짧은 화살표를 표시한다. 화살표는 `strokeWidth=5`, 불투명도 약 `0.62`로 방향을 명확히 하되, 바깥점 도착 후 중앙 복귀는 별도 대기 없이 연속으로 진행한다.
- `X` 종료 후 Phase 2 진입 전에 첫 고정점을 1500ms 동안 깜빡이며 고정점 응시를 안내하고, 이를 첫 9-point preview로 재사용한다. 이후 고정점은 1초 preview, 300ms 안정화, 500ms `rawGaze` 수집을 유지한다.
- phase 안내와 방향 화살표 표시 중에는 샘플을 수집하지 않고, 이동 중에는 `rawGaze`와 150ms 지연 정렬된 target을 수집한다. 복귀 구간에는 별도 안내 문구를 표시하지 않는다.
- 9-point 반복 샘플은 점별 median 기준으로 이상치를 제거하며, `+`·`X`·9-point 정제 샘플을 작은 MLP의 화면 좌표 보정 학습에 함께 사용한다.
- 보정 결과는 frame 단순 분할이 아닌 source·point block validation으로 평가하고 validation pixel error를 기록한다.
...
- Heatmap bin 누적
- 시선 EMA 평활화, `+`·`X` 중앙 왕복 캘리브레이션, 고정 3×3 9-point 캘리브레이션
- 질문 이동 시 결과 보존
- 질문별 결과 렌더링
- 측정값 없는 항목 표시
...
Step 4  VAD·발화 지표
Step 5  시선 안정화·중앙 왕복 경로 및 고정 3×3 9-point 캘리브레이션·Heatmap
...
- [ ] 발화 속도가 출력된다.
- [ ] 질문별 오디오 활동 타임라인이 출력된다.
- [ ] 발화·무음·긴 무음 구간과 무음 비율·긴 무음 횟수가 시각화된다.
- [ ] 질문별 시선 Heatmap이 출력된다.
- [ ] 저화질·원거리 입력에 대비한 시선 EMA 평활화가 적용된다.
- [ ] `+`·`X` 중앙 왕복 경로와 고정 3×3 9-point 샘플을 함께 사용하는 MLP 화면 좌표 보정이 적용된다.

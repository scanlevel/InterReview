...
- 캘리브레이션은 가상 면접관 영역 안의 고정 3×3 그리드 9점을 사용한다.
- 점을 이동시키지 않고 강조된 칸을 응시하게 하며, 안정화 이후 각 칸에서 반복 샘플을 수집한다.
- 각 목표점의 반복 샘플은 MAD로 이상치를 제거하고 2D Ridge 회귀로 화면 좌표 보정을 만든다.
...
Step 4  VAD·발화 지표
Step 5  시선 안정화·고정 3×3 격자 캘리브레이션·Heatmap
...
- [ ] 고정 3×3 격자 캘리브레이션과 반복 샘플 기반 2D Ridge 화면 좌표 보정이 적용된다.

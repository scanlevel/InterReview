...
| B-5 시선·Heatmap | 부분 충족 | 고정 3×3 격자, 2D Ridge 보정, EMA 평활화, 질문별 Heatmap이 연결됐다. 실제 장치 수집은 확인했으나 사람의 9점 응시 정확도 검증은 남아 있다. |
...
frontend gaze: 9 passed
frontend audio: 2 passed
ESLint: passed
Next.js production build: passed
git diff --check: passed
~~~

실제 장치·외부 API E2E는 `http://localhost:3000`에서 테스트 전용 Chrome 프로필과 실제 기본 장치를 사용했다.
...
3. 사람이 3×3 아홉 칸을 직접 응시하는 캘리브레이션 정확도 확인이 필요하다.

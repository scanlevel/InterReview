...
/** 현재 입력 형식을 구조화 자소서 문항으로 정규화한다. */
export function currentItems(draft: EssayDraft): EssayQAItem[] {
  if (draft.mode === "qa") return activeItems(draft);
  const answer = draft.free.trim();
  return answer ? [{ question: "", answer }] : [];
}

/** 회사 질문을 제외하고 지원자 답변만 근거 텍스트로 결합한다. */
export function answerText(items: EssayQAItem[]): string {
  return items.map((item) => item.answer).join("\n\n");
}
